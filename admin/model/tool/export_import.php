...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$nalichie = $this->db->escape($product['nalichie']);
		...
		$sql  = "INSERT INTO `".DB_PREFIX."product` (`product_id`,`quantity`,`nalichie`,`size`,`sku`,`upc`,";
		...
		$sql .= "($product_id,$quantity,'$nalichie','$size','$sku','$upc',";
		$sql .= in_array('ean',$product_fields) ? "'$ean'," : "";
		$sql .= in_array('jan',$product_fields) ? "'$jan'," : "";
		$sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
		$sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$nalichie = $this->getCell($data,$i,$j++,'');
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array
		( "product_id", "name", "categories", "main_category", "nalichie", "size", "sku", "upc" ); //added
		if (in_array("ean",$product_fields)) {
...
//		}
	...
	function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('nalichie'),15)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('size'),15)+1);
